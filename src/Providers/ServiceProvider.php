...
use Santanu\Lumen_Oauth\Storage\MongoDB;
...
class ServiceProvider extends LumenServiceProvider
{
    ...
    public function register() {
        ...
        $this->app['auth']->viaRequest('oauth', function () use ($userModel) {
            $server = new Server(new MongoDB($userModel), ['allow_implicit' => true]);
            $request = Request::createFromGlobals();
            if ($server->verifyResourceRequest($request)) {
                $response = $server->getAccessTokenData($request);
				return $userModel->findByAccessToken($response->access_token);
            }
        });
